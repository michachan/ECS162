...
function loadImageList(){
	...
	db.all('SELECT * FROM photoTags', function(err,rows){
		rows.forEach(function(row){
			if(n < 989){ //TODO Verify this is the correct #
				n++;
				console.log(row);
				var ref = row.idNum;
...
function annotateImage(APIrequestObject) {
	...
	function dbCallback(landmark,o){
		...
		db.serialize(function(){
			...
			if (landmark) {
				var query = `UPDATE photoTags SET landmark = "${landmark}", tags = "${o}" WHERE idNum = ${id} `
			} else {
				var query = `UPDATE photoTags SET tags = "${o}" WHERE idNum = ${id} `
			}

			db.run(query, dataCallback);
...
//Do it!
>>>>>>> 030dd02524a5e505e43182240f12b430cba949fa
//annotateImage();